...
This class includes different Knapsack strategies to
find the best scored subset from the given 
experiment list including highest rating, highest 
score, and lightest weight.

Sources:
- Greedy knapsack strategies (ratio, value, weight):
  GeeksforGeeks "Difference Between Greedy and 0/1 Knapsack"
  https://www.geeksforgeeks.org/difference-between-greedy-knapsack-and-0-1-knapsack-algorithms/
- Greedy knapsack Java implementation:
  ankurm.com "Implementing Greedy Knapsack Algorithm in Java"
  https://ankurm.com/implementing-greedy-knapsack-algorithm-in-java/
- Fractional knapsack greedy approach (sort and select pattern):
...
public class Strategies {
     ...
     * 
     * @param experiments   The available experiments
     * @param weight        Maximum weight value
     * @return  highestRated containing highest rated experiments whilst under specified weight.
     */
    public static List<Experiment> highestRating(List<Experiment> experiments, int weight) {
        ...
        // Choose the top items, skipping any that would exceed capacity
        List<Experiment> highestRated = new ArrayList<>();
        int currentWeight = 0;
        for (Experiment experiment : sortedRatings) {
            if (currentWeight + experiment.getWeight() <= weight) {
    ...
    public static List<Experiment> lightestWeight(List<Experiment> experiments, int weight) {
        // Create list sorted by weight ascending
